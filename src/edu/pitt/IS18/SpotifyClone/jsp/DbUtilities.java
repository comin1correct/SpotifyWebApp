    ...
    public void closeDbConnection()
    {
        try {
            if(conn != null){ // Check if connection object already exists
                conn.close(); // If does not exist, create new connection
            }
...
//            ErrorLogger.log(sql); // Log INSERT, UPDATE, DELETE query
     ...
     * @param rs - ResultSet from PreparedStatement 
     * @return a model for JTable
     * @throws SQLException
     */
    public DefaultTableModel getDataTable(ResultSet rs) throws SQLException{
...
default constructor - DbUtilities --> createDbConnection()
constructor  -  new database connection [tring hostName, String dbName, String dbUserName, String dbPassword]

createDbConnection() - Creates database connection using credentials stored in class variables.
getResultSet(String sql) - Get SQL result set (data set) based on an SQL query
executeQuery(String sql) -  Executes INSERT, UPDATE, DELETE queries
getDataTable(String sql) - Creates a model for JTable using default database table column names as table headers
getDataTable(String sqlQuery, String[] customColumnNames) - Creates a model for JTable using custom column names
